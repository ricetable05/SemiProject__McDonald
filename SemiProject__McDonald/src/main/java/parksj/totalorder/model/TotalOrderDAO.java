...
public class TotalOrderDAO implements InterTotalOrderDAO {
	 ...
	 * 
	 * @Override public List<TotalOrderVO> orderDetailList(int odr_no) throws
	 * SQLException {
	 * 
	 ...
	 * 
	 * String sql =
	 * "select O.odr_no, O.fk_userid, O.is_delivery, O.odr_date, O.delivery_time, O.is_delivery_price, O.delivery_loc, O.total, "
	 * +
	...
	public List<OrderDetailVO> orderDetail(int odr_no) throws SQLException {
		...
		try {
			...
			String sql = "select odr_product_no, fk_odr_no, fk_item_no, item_name, item_price, quantity, is_set "
					   + "from tbl_order_list "
					   + "where fk_odr_no = ? "
					   + "order by odr_product_no asc";
